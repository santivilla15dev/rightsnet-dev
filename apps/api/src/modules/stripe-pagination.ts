...
/**
 * V2 thin Events list uses opaque `page` tokens + next_page_url (not starting_after).
 * Items expose ISO `created` strings.
 */
export async function scanThinEventPages<T extends { id: string; created: string }>(options: {
  environment: string;
  accountRef: string;
  maxPages: number;
  initialSince?: number;
  fetch: (params: {
    ...
    created?: { gte: number };
    limit: number;
  }) => Promise<{ data: T[]; next_page: string | null }>;
  consume: (item: T) => Promise<void>;
}) {
  const kind = 'thin_events' as const;
  const { environment, accountRef } = options;
  const lock = await pool.connect();
  const lockKey = `stripe-scan:${environment}:${accountRef}:${kind}`;
  try {
    await lock.query('SELECT pg_advisory_lock(hashtextextended($1,0))', [lockKey]);
    ...
    let pageToken: string | null = cursor?.scan_after ?? null;
    const since = pageToken
      ? cursor.scan_since === null
        ? undefined
        : Number(cursor.scan_since)
      : cursor?.cursor_created_at
    ...
    for (let page = 0; page < options.maxPages; page++) {
      ...
      if (!result.data.length && result.next_page) throw new Error('STRIPE_EMPTY_PAGE_WITH_MORE');
      for (const item of result.data) {
        await options.consume(item);
        const createdUnix = Math.floor(new Date(item.created).getTime() / 1000);
        if (newestCreated === null || createdUnix > newestCreated) {
          newestCreated = createdUnix;
          newestRef = item.id;
        }
      }
      const complete = !result.next_page;
