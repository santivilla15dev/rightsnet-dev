...
import { z } from 'zod';
import { audit, withRlsActor, type DB, type RlsActor } from '../../../../packages/db/index.js';
...
/** Map product Actor → Postgres RLS GUCs (campaign pilot wiring). */
export function campaignRlsActor(user: Actor): RlsActor {
...
export async function listCampaigns(user: Actor, query: unknown) {
  ...
  return withRlsActor(campaignRlsActor(user), async (db) => {
    const can_edit = await campaignAccess(db, user, data.organization_id);
    const rows = (
      await db.query(
...
export async function createCampaign(user: Actor, body: unknown, key?: string) {
  ...
  await withRlsActor(rls, (db) => campaignAccess(db, user, data.organization_id, true));
  return mutate(
    user.id,
    'campaigns',
    key,
